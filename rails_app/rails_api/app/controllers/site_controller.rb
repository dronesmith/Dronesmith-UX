...
include HTTParty
    ...
    def login
      ...
      user = User.find_by(email: inputted_email)
        if user
          ...
          if user.authenticate(Base64.decode64(inputted_password))     
   ...
   #Verify User code  
    ...
    def send_password(state)
      ...
      @user.save
      uri_path = "https://api.sendgrid.com/v3/templates/#{ENV['SENDGRID_PASS_TEMPLATE_ID']}"
      response=HTTParty.get(uri_path, {   
      headers: {"Authorization": "Bearer #{ENV['SENDGRID_API_KEY']}"}
      })
      ...
      @final_body= html_body.gsub!('[%firstname%]', @user.first_name) 
      @final_body= html_body.gsub!('[%email%]', @user.email) 
      second_response= HTTParty.post("https://api.sendgrid.com/v3/mail/send",  
        { body: {
        "personalizations": [{"to": [{"email": @user.email}] }] ,
        "from": {"email": ENV['DS_EMAIL'] },
        "subject": "Your Dronesmith password ",
        "content": [{"type": "text/html", "value": @final_body}]
        }.to_json,  
        headers: {'Content-Type': 'application/json', "Authorization": "Bearer " + ENV['SENDGRID_API_KEY']} }
      )    
